...
class ProviderFailoverManager {
  constructor() {
    this.providers = [
      { name: 'nvidia', priority: 1, healthy: true, enabled: true, circuitBreaker: { state: 'closed', failures: 0 } },
      { name: 'groq', priority: 2, healthy: true, enabled: true, circuitBreaker: { state: 'closed', failures: 0 } },
      { name: 'zai', priority: 3, healthy: true, enabled: true, circuitBreaker: { state: 'closed', failures: 0 } },
      { name: 'opencode', priority: 4, healthy: true, enabled: true, circuitBreaker: { state: 'closed', failures: 0 } },
      { name: 'openrouter', priority: 5, healthy: true, enabled: true, circuitBreaker: { state: 'closed', failures: 0 } }
    ];
...
function startTelegramBot() {
  ...
  bot.on('message:text', async (ctx) => {
    ...
    // Check rate limit
    let userData = userMessageCounts.get(userId) || { count: 0, resetTime: now + 60000 };
  ...
  bot.command('providers', async (ctx) => {
    try {
      const response = await fetch(`${process.env.PROXYOS_BACKEND_URL || `http://localhost:${PORT}`}/api/providers`);
      const data = await response.json();
